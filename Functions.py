...
def warp_img(img,threshold_value,border_offset_px,show_outer_edge,printsize):
...
def extraction_canny(img,factor_epsilon,threshold_value,border_offset_px,every_nth_point,connectpoints,printsize,printpoints,show_outer_edge):
    ret,thresh=cv2.threshold(img,threshold_value,255,0)
    contours,hierarchy = cv2.findContours(thresh,cv2.RETR_CCOMP,cv2.CHAIN_APPROX_SIMPLE)
